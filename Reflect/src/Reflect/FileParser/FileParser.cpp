...
namespace Reflect::Parser
{
	...
	bool FileParser::ParseFile(FileParsedData& fileData)
	{
		...
		do
		{
			...
			if (containerToBeReflected)
			{
				...
				{
					Log_Error("[FileParser::ParseFile] Something has gone wrong parsing file '%s'. All reflect data has been removed. Please fix issues.", fileData.FullPath.data());
	...
	uint64_t FileParser::FindNextContainer(FileParsedData& fileData) const
	{
		...
		//const uint64_t fwdDeclareIdx = static_cast<size_t>(fileData.Data.find(";", containerStartIdx));
		//const uint64_t leftBracketIdx = static_cast<size_t>(fileData.Data.find("{", containerStartIdx));

		//if (fwdDeclareIdx < leftBracketIdx)
		//{
		//	// Forward delectations can't be relfected/parsed.
		//	fileData.Cursor = fwdDeclareIdx + 1;
		//	goto FuncStart;
		//}
	...
	void FileParser::CheckStringViewBounds(const FileParsedData& fileData, size_t cursor, std::string_view view) const
	{
		...
		{
			Log_Error("[FileParser::CheckStringViewBounds] File: '%s', Line: '%s', Cursor: '%d'.", fileData.FileName.data(), view.data(), cursor);
			exit(0);
		}
	}
